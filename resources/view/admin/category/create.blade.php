...
@section('head-tag')
    <title>ادمین | ایجاد دسته بندی جدید</title>
@endsection
                                    ...
                                    <form class="row" action="<?= route('admin.category.store') ?>" method="post" enctype="multipart/form-data">
                                        <div class="col-md-6">
                                            <fieldset class="form-group">
                                                ...
                                                <input value="<?= old('name') ?>" name="name" type="text" id="helperText" class="form-control <?= errorClass('name')  ?>"
                                                    placeholder="نام ...">
                                                    <?= errorText('name') ?>
                                            </fieldset>
                                        </div>
                                            ...
                                            <fieldset class="form-group">
                                                <div class="form-group">
                                                    ...
                                                    <select name="parent_id" class="select2 form-control <?= errorClass('name')  ?>">
                                                        ...
                                                        <option value="<?= $category->id ?>" <?= old('parent_id') == $category ? 'selected' : ''?>><?= $category->name ?></option>
                                                        <?php } ?>
                                                    </select>
                                                    <?= errorText('parent_id') ?>
                                                </div>
