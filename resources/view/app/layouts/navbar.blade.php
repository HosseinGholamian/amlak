<nav class="navbar navbar-expand-lg navbar-dark ftco_navbar bg-dark ftco-navbar-light" id="ftco-navbar" style="direction: rtl;">
    <div class="container">
        <a class="navbar-brand" href="<?= route('index.home') ?>">Royal<span>estate</span></a>
        ...
        <div class="collapse navbar-collapse" id="ftco-nav">
            <ul class="navbar-nav ml-auto">
                <li class="nav-item  <?=  (currentUrl() == route('index.home')) ? 'active' : '' ?>"><a href="<?= route('index.home') ?>" class="nav-link">خانه</a></li>
                <li class="nav-item   <?=  (currentUrl() == route('all.ads.home')) ? 'active' : '' ?> "><a href="<?= route('all.ads.home') ?>" class="nav-link">آگهی ها</a></li>
                <li class="nav-item   <?=  (currentUrl() == route('about.home')) ? 'active' : '' ?>"><a href="<?= route('about.home') ?>" class="nav-link">درباره ما</a></li>
                <li class="nav-item   <?=  (currentUrl() == route('all.post.home')) ? 'active' : '' ?>"><a href="<?= route('all.post.home')  ?>" class="nav-link">بلاگ</a></li>
                <?php if(\System\Auth\Auth::checkLogin()) { ?>
                    <?php if(\System\Auth\Auth::user()->user_type == 'admin') { ?>
                        ...
                        <?php } ?>
                        <li class="nav-item btn-primary"><a href="<?= route('index.home') ?>" class="nav-link ml-lg-1 mr-lg-5 text-dark"><span class="icon-user m-2"></span> <?= \System\Auth\Auth::user()->first_name . ' ' . \System\Auth\Auth::user()->last_name ?></a></li>
